<template>
  <div class="card">
    ...
    <div class="description">
      ...
      <div v-else>
        <button @click="increaseQuantity">+</button><strong class="quantity">{{quantity}}</strong><button @click="decreaseQuantity">-</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    increaseQuantity(){
      if(this.quantity == this.product.stock){ //Quantity is equal to stock 
  ...
  mounted(){ //check itemStatus on every re-render
    this.$emit('update:cartQuantity', this.quantity); //update value(prop) in parent component
    if(this.quantity > 0){
        this.itemStatus = false;
      }
