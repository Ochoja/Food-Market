<template>
  ...
  <div class="site_description">Fruit Market</div>

  <h1>Browse</h1>
  <div :key="reset" class="shelf">
    <!--bind key force rerender when variable value changes -->
    <items
      v-for="(item, index) in shelf"
      :key="index"
      :product="item"
      v-model:cartQuantity="item.quantity"
      @add="addtoCart(item)"
      @remove="removeItem(item.cartIndex)"
      @update="updateCart(item.cartIndex)"
    ></items>
  </div>
  ...
  <transition name="slide-fade">
    <div v-if="toggleCart" class="cart">
      <h1 class="cart_head">Cart</h1>
      <span class="close" @click="showCart()"></span>
      ...
      <div v-for="(item, index) in cart" :key="index" class="cartItem">
        ...
        <span class="itemQty"
          >{{ item.quantity }}
          <span class="cart_remove" @click="removeItem(item.cartIndex)"
            ><fa icon="trash-alt" class="icon" /></span
        ></span>
      </div>
...
export default {
  ...
  methods: {
    showCart() {
      //toggles cart with the help of v-if and this.toggleCart
